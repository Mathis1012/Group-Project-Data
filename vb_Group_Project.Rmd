---
title: "Group Project, Immunology - Influenza"
author: "Mathis Mortier, Femke Van Hoenacker, Bleyke van Denderen, Maxence Cayzeele"
date: "2024-11-14"
output: 
  html_document:
    toc: true
...
head(influ_exp_name)
```
After head/print influ_exp_name, we manually compared a few of the data values with the data values before the pivot function to see if they were the same. This was the case.
...
```
We see that there are a lot of missing values for the cell types. At least these are all missing in 54% of the cases. These L50s are all molecules and are missing for at least 29% of the cases.
...
```
We see that there are much less missing values in the metadata. For example visit_age has 0% missing values. But vaccine_response has 50% missing values.
...
When the summary_data is printed, the percentages of every age category for low vaccine response = 0, high vaccine response = 1 and NA is visualised. Based on this, we decided to not use the highest age categories for our hypothesis, because they contain too much missing values. Imputation cannot be accomplished because we cannot determine whether the person is a low or high responder based on the age of the donor. Therefore, the missing values are MCAR (missing completely at random). To avoid bias, the data of donors without a value for vaccine response, is not used. This is done using the "na.omit()" function.

The age category "31-40" contains only data from 20 donors (10 with low responders, 7 with high responders and 3 are missing values). This group was considered to be too small and is therefore also not included for our hypothesis. To summarize only the age categories "0-10", "11-20" and "21-30" will be selected for the hypothesis testing.
...
## Visualisation after statistical testing
```{r}
library(scales)
...
ggplot(summary_data, aes(x = age_cat, y = percentage, fill = response)) +
  ...
  annotate("text", x = Inf, y = Inf, label = paste("P-value =", round(p_value, 4)), 
           hjust = 4, vjust = 0.9, size = 4, color = "black")
```
...
The p-value (0.3339) is greater than the significance level of 0.05. This indicates that there is no statistically significant association between age categories and vaccine response for these three age categories ("0-10", "11-20" and "21-30"). In other words, the differences in vaccine response across these age categories are likely due to random chance rather than a real effect.
...
# Research question 2: Which type of cells are different in low vs high vaccine responders group?
...
#high responders will be refered as responders dataset

influ_HP_non_vaccine_responders <- influ_HP %>% 
  filter(vaccine_response == 0)%>%
   select(-vaccine_response)
#low responders will be refered as non-responders dataset
...
To solve the problem of MAR missing values, imputation is performed. That is why the missing values are changed into the mean of the available values. The mean of each cell type in both datasets (high responders = "responders dataset" and low responders = "non-responders dataset") is calculated and those means replace the missing values for that specific cell type.
...
head(influ_HP_non_vaccine_responders)
```
### Looking at the differences of the cell types between high and low vaccine responders

#### Making a new dataset with the difference of mean values of all columns (=cell types) between high vaccine responders and low vaccine responders
...
In the following code, "lef_join()" is used. This function ensures that all rows from the left dataset (mean_divided in this case) are preserved, even if there are no matching rows in the right dataset (missing_values_exp), which is here not the case (both datasets contain the same number of rows). If a key (celtype) exists in mean_divided but not in missing_values_exp, the columns brought in from missing_values_exp will contain NA for that key.
...
The cell types with mean_divided values that are the furthest from 1 are the most interesting. This is because the mean of responders dataset was divided by the mean of non-responders dataset and when there is no difference in mean between the high responders and low responders group, this ratio is 1.
...
### Visualisation of the differences of CD161_pos_CD45RA_pos_Tregs and HLADR_neg_CD38_pos_CD4_pos_T_cells
To visualize the cells we need to combine the imputated responder and non-responder dataset and add then the vaccine_response column again. By doing this we can visualize the differences for both cell types.

#### Making a combined dataset with the imputed data of the high and low vaccine responders
```{r}
...
#### Visualisation of the differences in the cell types between the high and low responders
...
combined_data %>%
  ...
  geom_boxplot()+
  labs(title = "High vs low responders CD161_pos_CD45RA_pos_Tregs", x = "Vaccine response", y = "CD161_pos_CD45RA_pos_Tregs (% of parent)")+
...
We see that there are enough datapoints to conduct a statistical test. For the low responder group 252 datapoints and for the high responder group 111 datapoints.
...
combined_data %>%
  ...
  geom_boxplot()+
  labs(title = "High vs low responders HLADR_neg_CD38_pos_CD4_pos_T_cells", x = "Vaccine response", y = "HLADR_neg_CD38_pos_CD4_pos_T_cells (% of parent)")+
  scale_color_manual(values = c("#E69F00", "#56B4E9"), #color-blind friendly colors
                    labels = c("0" = "Low responder", "1" = "High responder"))+
  theme_minimal()
```
...
Null hypothesis: 
There is no difference in amount of CD161_pos_CD45RA_pos_Tregs and HLADR_neg_CD38_pos_CD4_pos_T_cells between the high and low vaccine responders.
H0:P(Low responder∣CD161_pos_CD45RA_pos_Tregs + HLADR_neg_CD38_pos_CD4_pos_T_cells)=P(Low responder)
 
Alternative hypothesis:
There is a difference in amount of CD161_pos_CD45RA_pos_Tregs and HLADR_neg_CD38_pos_CD4_pos_T_cells between the high and low vaccine responders.
...
Before we test these cell types, we need to control if the data are normally distributed and standardized. This is done by looking at the QQ-plot. If the data is not normally distributed, we need to log-transform the data. If the data is not on the same scale, we need to standardise the data. 
...
Data is not normally distributed, so we need to log transform the data (especially HLADR_neg_CD38_pos_CD4_pos_T_cells). We see both times a skewing at the right side of the QQ-plot.
...
```{r}
#printing the standarddeviation of both cell types using the skim() function.
combined_data %>%
  group_by(vaccine_response)%>%
...
Note: The values of the cell types become unitless after the standardization. That is why there is no unit for the data of the cell types in both boxplots.
...
combined_data_standardized %>%
  ...
  geom_boxplot()+
  labs(
    title = "High vs low responders CD161_pos_CD45RA_pos_Tregs",
    x = "Vaccine Response",
    y = "log(CD161_pos_CD45RA_pos_Tregs)"
  ) +
  scale_color_manual(
...
combined_data_standardized %>%
  ...
  geom_boxplot()+
  labs(
    title = "High vs low responders HLADR_neg_CD38_pos_CD4_pos_T_cells",
    x = "Vaccine Response",
    y = "log(HLADR_neg_CD38_pos_CD4_pos_T_cells)"
  ) +
  scale_color_manual(
    values = c("#E69F00", "#56B4E9"),  # Color-blind friendly colors
    labels = c("0" = "Low Responder", "1" = "High Responder")
...
Now we investigate the R-squared.
```{r}
# R-squared
1 - logistic_model$deviance / logistic_model$null.deviance
```
Although we have one celtype that is significant we can see that the R-squared has a low value (0.084). This means that our model is of low quality. 

```{r}
#Confidence interval of 95% of the log-odds (log-odds is the result of the logistic model)
...
### Visualization of log odds and odds ratios of the two cell types

Visualization of the log odds of both cell types
```{r}
# Extract coefficients
coefficients <- summary(logistic_model)$coefficients
log_odds <- coefficients[-1, "Estimate"]  # Exclude the intercept
conf_intervals <- confint(logistic_model, level = 0.95)[-1,]  # Exclude the intercept
p_values <- coefficients[-1, "Pr(>|z|)"]  # Exclude the intercept

# Create a data frame for plotting
results <- data.frame(
  Variable = names(log_odds),
  log_odds = log_odds,
...
#Visualization of the log odds and 95% confidence intervals of the two cell types
ggplot(results, aes(x = reorder(Variable, log_odds), y = log_odds)) +
  geom_point(size = 3, color = "red") +
  geom_errorbar(aes(ymin = LowerCI, ymax = UpperCI), width = 0.2, color = "black") +
  geom_hline(yintercept = 0, linetype = "dashed", color = "grey") +
  geom_text(aes(label = ifelse(pValue < 0.05, paste0("p = ", format(pValue, nsmall = 4)), paste0("p = ", round(pValue, 4)))), 
            hjust = -0.2, vjust = 0.5, size = 3) +
  labs(title = "Log Odds and 95% Confidence Intervals of the Two Cell Types",
       x = "Cell Type",
       y = "Log Odds") +
  theme_minimal() +                              
...
Odds ratio visualization of both cell types
...
Intercept: The intercept is -0.9087, which is statistically significant (p < 0.001). This represents the log-odds of being a high vaccine responder when both predictors are zero.

Conclusion:
CD161_pos_CD45RA_pos_Tregs: This variable has a positive log odds of 0.6367 (95%CI [0.4042689, 0.879381202]) and is highly significant (p < 0.001). This suggests that higher levels of CD161_pos_CD45RA_pos_Tregs are associated with increased odds of being a high vaccine responder. The confidence interval does not include zero, reinforcing the significance. 

The odds ratio is approximately 1.89 (95%CI [1.4982067, 2.4094083]). This means that for each unit increase in CD161_pos_CD45RA_pos_Tregs, the odds of being a high vaccine responder increase by about 89%. The confidence interval does not include one, reinforcing its significance.


HLADR_neg_CD38_pos_CD4_pos_T_cells: This variable has a negative log odds of -0.2619 (95%CI [-0.5476484, 0.008808105] and is marginally significant (p ≈ 0.064). This suggests that higher levels of HLADR_neg_CD38_pos_CD4_pos_T_cells might be associated with decreased odds of being a high vaccine responder, but the evidence is not strong. The confidence interval includes zero, again indicating that the effect is not significant.

The odds ratio is approximately 0.77 (95%CI [0.5783082, 1.0088470]). This means that for each unit increase in HLADR_neg_CD38_pos_CD4_pos_T_cells, the odds of being a vaccine responder decrease by about 23%. The confidence interval includes one, again indicating that the effect is not significant.
...
# Machine Learning
The machine learning model was made in python code (see other file). Here we just made the used dataset. This data set contains all the cell types that are expressed in "% of Parent" and the vaccine response. This is done to ensure that the data is on the same scale and can be used for the machine learning model. Missing data of those cell types are also being imputated by the mean of that cell type as seen before under Research question 2. 

## Preprocessing of the data

### Making new experimental and metadata dataset with only cell types in "% of Parent" unit
...
Controlling the amount of missing values for each celltype within the (% of Parent) influ_exp_name_Parent dataset. This is done to see which cell types have the least amount of missing values. This is done to see which cell types are the most interesting to use for the machine learning model and avoid bias.

```{r}
#Calculating the amount of missing values per celtype before imputation
missing_values_exp_P <- influ_exp_name_Parent %>%
  summarise_all(~sum(is.na(.))/n())%>%
  gather() %>%
  arrange(-value) #Sorting from high to low
missing_values_exp_P
```
We can see that most of the cell types have a high amount of missing values (>0.92). We will exclude these cell types from the machine learning model, because these could introduce bias. We will set the cut-off at 0,5 (or 50%) of missing values. This means that we will only include the cell types that have less than 50% of missing values for the machine learning model.

```{r}
#Making a list with all the celtypes that have less than 50% of missing values
celltypes_P <- missing_values_exp_P %>%
  filter(value < 0.5)%>%
  select(key)%>%
  pull()
length(celltypes_P)
...
We will use 76 cel types to train the machine learning model (77 - 1, because donorID is in the dataset but is not a cell type!). We are aware that we exclude a lot of cell types (initially 400+) that potentially could show a significant association with the vaccine response. However, we want to avoid bias in the model and therefore only include the cell types that have less than 50% of missing values.

```{r}
#Filtering the influ_exp_name_Parent dataset to only include the cell types that have less than 50% of missing values (celltypes_P)

influ_exp_name_Parent <- influ_exp_name_Parent %>%
  select(donor_id, celltypes_P)
head(influ_exp_name_Parent)
summary(influ_exp_name_Parent)
```
...
influ_HP_vaccine_responders_P <- influ_exp_name_Parent %>%
  filter(vaccine_response == 1)%>%
  select(-vaccine_response)
#High responders will be refered to as responder dataset

influ_HP_non_vaccine_responders_P <- influ_exp_name_Parent %>%
  filter(vaccine_response == 0)%>%
   select(-vaccine_response)
#Low responders will be refered to as non-responder dataset
...
### Making a combined dataset with the imputed data of the high and low vaccine responders for the machine learning model
